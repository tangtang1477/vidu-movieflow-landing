      ...
      <div className="mx-auto max-w-[1400px] px-8 py-24">
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-3">
          {tiers.map((t) => (
            ...
            >
              {t.featured && (
                <div className="absolute right-6 top-6 rounded-full border border-primary/40 bg-primary/10 px-2.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-primary">
                  Featured
                </div>
              )}
...
function CreatorChallenge() {
  const joinSteps = [
    "Join the #vidu-shortsrush-showcase channel and DM to register",
    "Create a cinematic AI short using Vidu workflow on MovieFlow",
    "Post it on TikTok, YouTube Shorts, Instagram Reels, or X.",
    "Tag @MovieFlow and @ViduAI.",
    "Use hashtags: #MovieFlowAI #ViduAI #AIShorts #AIMonetization",
    "Submit your link in the showcase channel or via the event form",
  ];
  ...
  const rewards = [
    ...
    { t: "Runner-Up", v: "$300 × 2" },
    { t: "Best Hook", v: "$200 × 3" },
    { t: "Best Vidu Execution", v: "$150 × 5" },
    { t: "Community Resonance Award", v: "$100 × 10" },
    { t: "Participation Credits", v: "2000 credits × 20" },
  ];
  ...
  return (
    <section id="examples" className="border-b border-border bg-background">
      <div className="mx-auto max-w-[1400px] px-8 py-24">
        <div className="max-w-3xl">
          <SectionLabel>Creator Challenge</SectionLabel>
          <h2 className="mt-6 text-[44px] font-bold tracking-tight md:text-[56px] leading-[1.05]">
            MovieFlow × Vidu Official Creator Challenge
            <span className="block text-primary">Shorts to Spotlight One-Dollar Movie Sprint</span>
          </h2>
          <p className="mt-6 text-lg text-muted-foreground">
            Use Vidu's powerful video generation models and MovieFlow's short-form creation workflow to turn a simple idea into a cinematic AI video and join the official creator challenge.
          </p>
        ...
        <div className="mt-16 grid gap-6 lg:grid-cols-2">
            ...
            <ol className="mt-6 space-y-4">
              {joinSteps.map((step, i) => (
                <li key={i} className="flex gap-4">
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-primary/40 bg-primary/10 font-mono text-xs text-primary">
                    {i + 1}
                  </span>
          ...
          {/* Reward Pool */}
          <div className="rounded-2xl border border-primary/30 bg-gradient-to-br from-primary/10 via-card to-card p-8 shadow-[0_0_60px_-20px_var(--primary)]">
            <SectionLabel>Reward Pool</SectionLabel>
            <h3 className="mt-4 text-2xl font-semibold tracking-tight">
              <span className="text-primary">$5,000</span> Creator Reward Pool
            </h3>
            ...
            <ul className="mt-6 divide-y divide-border/60">
              {rewards.map((r) => (
                <li key={r.t} className="flex items-center justify-between py-3">
                  ...
                  <span className="font-mono text-sm text-primary">{r.v}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="mt-6 rounded-2xl border border-border bg-card p-8">
          ...
          <p className="mt-3 font-mono text-sm uppercase tracking-wider text-primary">
            Create. Post. Test. Repeat.
          </p>
          <p className="mt-2 text-sm text-muted-foreground">
            Who is ready to build their AI shorts workflow?
          </p>
        </div>
          ...
          <div className="mt-6 grid gap-px overflow-hidden rounded-2xl border border-border bg-border sm:grid-cols-2 lg:grid-cols-3">
            {faqs.map((f) => (
              <div key={f.q} className="bg-background p-6">
                <h4 className="text-sm font-semibold tracking-tight text-foreground">{f.q}</h4>
